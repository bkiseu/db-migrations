#!/bin/bash
# Script to set up the CodeCommit repository with initial migrations

# Configuration
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
ROOT_DIR="$(dirname "$SCRIPT_DIR")"
MIGRATIONS_DIR="${ROOT_DIR}/migrations"
REPO_NAME="$1"  # Pass the repository name as the first argument
REPO_URL="$2"   # Pass the repository URL as the second argument

if [ -z "$REPO_NAME" ] || [ -z "$REPO_URL" ]; then
  echo "Error: Repository name and URL must be provided"
  echo "Usage: $0 <repository_name> <repository_url>"
  exit 1
fi

echo "Setting up CodeCommit repository with initial migrations..."
...
if [ $? -ne 0 ]; then
  ...
  echo "Make sure your AWS credentials are configured correctly for CodeCommit."
  echo "You may need to set up the git-remote-codecommit helper."
  exit 1
fi
...
git add .
git config --local user.name "Terraform Setup"
git config --local user.email "terraform@example.com"
git commit -m "Initial commit with Flyway migration structure"

# Push to the main branch
git push origin master

# Optionally create a 'main' branch if it doesn't exist yet and the repo uses 'main' instead of 'master'
git checkout -b main
git push -u origin main

echo "Repository setup complete! The pipeline should trigger automatically."
